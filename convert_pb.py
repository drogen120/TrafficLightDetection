...
ckpt = tf.train.get_checkpoint_state(os.path.dirname('./logs_both/checkpoint'))

with tf.Graph().as_default() as graph:
    input_tensor = tf.placeholder(tf.float32, shape=(None, 448, 448, 3), name='input_image')
    with tf.Session() as sess:
        ssd_net = mobilenet_ssd_traffic.Mobilenet_SSD_Traffic()
    ...
    output_node_names = 'mobilenet_ssd_traffic_net/softmax_3/Reshape_1,mobilenet_ssd_traffic_net/softmax_2/Reshape_1,mobilenet_ssd_traffic_net/softmax_1/Reshape_1,mobilenet_ssd_traffic_net/softmax/Reshape_1'
    ...
    output_graph = './mobilenet_traffic_light.pb'
