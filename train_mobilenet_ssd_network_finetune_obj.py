...
tf.app.flags.DEFINE_string(
    'model_name', 'mobilenet_pretrained_obj', 'The name of the architecture to train.')
tf.app.flags.DEFINE_string(
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with slim.arg_scope(arg_scope):
            predictions, localisations, logits, obj_logits, end_points = \
                ssd_net.net(b_image, is_training=True)

        f_i = 0
        for predict_map in obj_logits:
            show_obj = slim.softmax(predict_map)
            predict_map = show_obj[:, :, :, :, 1:]
            ...
            summaries.add(tf.summary.image("pre_obj_map_%d" % f_i, tf.cast(predict_map, tf.float32)))
            f_i += 1
        f_i = 0
        for predict_map in predictions:
            ...
            predict_map = tf.reduce_max(predict_map, axis=3)
            predict_map = tf.expand_dims(predict_map, -1)
            summaries.add(tf.summary.image("pre_map_%d" % f_i, tf.cast(predict_map, tf.float32)))
            f_i += 1

        ssd_net.losses(logits, obj_logits, localisations, b_gclasses, b_glocalisations, b_gscores,1,
        ...
        arg_scope = ssd_net.arg_scope(weight_decay=FLAGS.weight_decay, data_format=DATA_FORMAT)
        with slim.arg_scope(arg_scope):
            predictions_k, localisations_k, logits_k, logits_obj_k, end_points_k = \
                ssd_net.net(b_image_k, is_training=True, reuse=True)

        f_i = 0
        for predict_map in logits_obj_k:
            predict_map = predict_map[:, :, :, :, 1:]
            predict_map = tf.reduce_max(predict_map, axis=4)
            predict_map = tf.reduce_max(predict_map, axis=3)
            predict_map = tf.expand_dims(predict_map, -1)
            summaries.add(tf.summary.image("k_pre_obj_map_%d" % f_i, tf.cast(predict_map, tf.float32)))
            f_i += 1
        ...
        ssd_net.losses(logits_k, logits_obj_k, localisations_k, b_gclasses_k, b_glocalisations_k, b_gscores_k, 2,
                       match_threshold=FLAGS.match_threshold,
                       negative_ratio=FLAGS.negative_ratio,
                       alpha=FLAGS.loss_alpha,
                       label_smoothing=FLAGS.label_smoothing)
