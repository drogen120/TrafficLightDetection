...
image_pre, labels_pre, bboxes_pre, bbox_img = ssd_vgg_preprocessing.preprocess_for_eval(
    img_input, None, None, net_shape, data_format, resize=ssd_vgg_preprocessing.Resize.SPECIAL_WQL)
...
ssd_net = mobilenet_pretrained_obj.Mobilenet_SSD_Traffic()
with slim.arg_scope(ssd_net.arg_scope(data_format=data_format, is_training=False)):
    ...
    for predict_map in predictions:
        ...
        class_list = tf.split(predict_map, 9, axis=4)
        class_index = 0
        for class_pre in class_list:
            if f_i < 2:
                predict_list = tf.split(class_pre, 6, axis=3)
                anchor_index = 1
                for anchor in predict_list:
                    summaries.add(tf.summary.image("class_%d_predicte_map_%d_anchor%d" % (class_index,f_i,anchor_index), tf.squeeze(tf.cast(anchor, tf.float32), axis=4)))
                    anchor_index += 1
            # else:
...
def process_image(img, select_threshold=0.3, nms_threshold=.45, net_shape=(1024, 1024)):
    # Run SSD network.
...
for image_name in image_names:
    ...
    # visualization.plt_bboxes(img, rclasses, rscores, rbboxes, index)
