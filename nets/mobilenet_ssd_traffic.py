...
_CONV_DEFS = [
    Conv(kernel=[3, 3], stride=2, depth=32),  # 224*224
    DepthSepConv(kernel=[3, 3], stride=1, depth=64), # 224*224
    DepthSepConv(kernel=[3, 3], stride=2, depth=128), # 112*112
    DepthSepConv(kernel=[3, 3], stride=1, depth=128), # 112*112
    DepthSepConv(kernel=[3, 3], stride=2, depth=256), # 56 * 56
    DepthSepConv(kernel=[3, 3], stride=1, depth=256), # 56 * 56
    DepthSepConv(kernel=[3, 3], stride=1, depth=256), # 56 * 56
    DepthSepConv(kernel=[3, 3], stride=2, depth=512), # 28 * 28
    DepthSepConv(kernel=[3, 3], stride=1, depth=512), # 28 * 28
    DepthSepConv(kernel=[3, 3], stride=1, depth=512), # 28 * 28
    DepthSepConv(kernel=[3, 3], stride=2, depth=512), # 14 * 14
    DepthSepConv(kernel=[3, 3], stride=1, depth=512), # 14 * 14
    DepthSepConv(kernel=[3, 3], stride=2, depth=512), # 7 * 7
    DepthSepConv(kernel=[3, 3], stride=1, depth=1024) # 7 * 7
]
class Mobilenet_SSD_Traffic(object):
    default_params = SSDParams(
        ...
        feat_layers=['Conv2d_6_pointwise', 'Conv2d_8_pointwise', 'Conv2d_11_pointwise', 'Conv2d_13_pointwise'],
    ...
    def net(self, inputs,
            ...
            reuse=None,
            scope='mobilenet_ssd_traffic_net'):
...
def mobilenet_ssd_base(inputs,
    ...
    with tf.variable_scope(scope, 'Mobilenet_SSD_traffic', [inputs]):
...
def mobilenet_ssd_traffic_net(inputs,
    ...
    with tf.variable_scope(scope, 'Mobilenet_SSD_traffic', [inputs, num_classes],
            ...
            for i, layer in enumerate(feat_layers):
                with tf.variable_scope(layer + '_box'):
                    if i < len(feat_layers) -1 :
                        ...
                        p, l = ssd_multibox_layer(redudent_feature,
                                                  num_classes,
                                                  anchor_sizes[i],
                                                  anchor_ratios[i],
                                                  normalizations[i])
