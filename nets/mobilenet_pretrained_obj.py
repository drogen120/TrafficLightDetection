...
class Mobilenet_SSD_Traffic(object):
    ...
    def losses(self, logits, logits_obj, localisations,
        ...
        """
        if part_num == 1:
            return ssd_losses_part1(logits, logits_obj, localisations,
                              gclasses, glocalisations, gscores,
            ...
            return ssd_losses_part2(logits, logits_obj, localisations,
                              gclasses, glocalisations, gscores,
                              match_threshold=match_threshold,
                              negative_ratio=negative_ratio,
                              alpha=alpha,
                ...
                for i, layer in enumerate(feat_layers):
                    with tf.variable_scope(layer + '_box'):
                        p, l, o = ssd_multibox_layer(end_points[layer],
                                                  ...
                                                  anchor_ratios[i],
                                                  normalizations[i])
                    pr_o = prediction_fn(o)
                    pr_p = prediction_fn(p)
                    predictions.append(pr_p * pr_o[:,:,:,:,1:])
                    logits.append(p)
                    localisations.append(l)
                    objpredictions.append(o)
...
def ssd_anchor_one_layer(img_shape,
    ...
    h[1] = sizes[0] / img_shape[0] * 0.8
    w[1] = sizes[1] / img_shape[1]

    di = 2

    for i, r in enumerate(ratios):
        h[i+di] = sizes[0] / img_shape[0] * (r-0.5)
...
def ssd_multibox_layer(inputs,
    ...
    num_obj_pred = num_anchors * 2
    obj_pred = slim.separable_conv2d(net, None, [3, 3], depth_multiplier=1, stride=1, rate=1, normalizer_fn=slim.batch_norm, scope='conv_obj_dipthwise')
    obj_pred = slim.conv2d(obj_pred, num_obj_pred, [1, 1], stride=1, normalizer_fn=slim.batch_norm, activation_fn=None, scope='conv_obj_pointwise')
    obj_pred = tf.reshape(obj_pred,
...
def ssd_losses_part1(logits, logits_obj, localisations,
               gclasses, glocalisations, gscores,
               match_threshold=0.5,
               negative_ratio=3.,
               alpha=1.,
    ...
    with tf.name_scope(scope, 'ssd_losses'):
        ...
        for i in range(2):
            flogits.append(tf.reshape(logits[i], [-1, num_classes]))
            flogits_obj.append(tf.reshape(logits_obj[i], [-1, 2]))
            fgclasses.append(tf.reshape(gclasses[i], [-1]))
            fgscores.append(tf.reshape(gscores[i], [-1]))
            flocalisations.append(tf.reshape(localisations[i], [-1, 4]))
            fglocalisations.append(tf.reshape(glocalisations[i], [-1, 4]))
        # And concat the crap!
        logits = tf.concat(flogits, axis=0)
        logits_obj = tf.concat(flogits_obj, axis=0)
        gclasses = tf.concat(fgclasses, axis=0)
        gscores = tf.concat(fgscores, axis=0)
        localisations = tf.concat(flocalisations, axis=0)
        glocalisations = tf.concat(fglocalisations, axis=0)
        gobjs = tf.cast(gclasses, tf.bool)
        gobjs = tf.cast(gobjs, tf.int32)
        dtype = logits.dtype
        ...
        # Hard negative mining...
        no_classes = tf.cast(pmask, tf.int32)
        predictions = slim.softmax(logits_obj)
        nmask = tf.logical_and(tf.logical_not(pmask),
                               gscores > -0.5)
        fnmask = tf.cast(nmask, dtype)
        nvalues = tf.where(nmask,
        ...
        # Add cross-entropy loss.
        with tf.name_scope('cross_entropy_pos_class'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                  labels=gclasses)
            loss = tf.div(tf.reduce_sum(loss * fpmask), batch_size, name='value')
            tf.losses.add_loss(loss)

        with tf.name_scope('cross_entropy_pos_obj'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_obj,
                                                                  labels=gobjs)
            loss = tf.div(tf.reduce_sum(loss * fpmask), batch_size, name='value')
            tf.losses.add_loss(loss)

        with tf.name_scope('cross_entropy_neg_obj'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_obj,
                                                                  labels=no_classes)
            loss = tf.div(tf.reduce_sum(loss * fnmask), batch_size, name='value')
            tf.losses.add_loss(loss)
...
def ssd_losses_part2(logits, logits_obj, localisations,
               gclasses, glocalisations, gscores,
               match_threshold=0.5,
               negative_ratio=3.,
               alpha=1.,
